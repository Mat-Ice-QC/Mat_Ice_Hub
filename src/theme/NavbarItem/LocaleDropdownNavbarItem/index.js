  ...
  const getBaseURLForLocale = (locale) => {
    ...
    if (isSameDomain) {
      ...
      try {
        ...
        if (alternateUrl) {
          return `pathname://${alternateUrl}`;
        }
      } catch (e) {
        // Fallback for when alternate utils fail
      }
      return `pathname://${locale === siteConfig.i18n.defaultLocale ? '/' : `/${locale}/`}`;
    }
    ...
    return locale === siteConfig.i18n.defaultLocale ? siteConfig.url : `${siteConfig.url}/${locale}/`;
  };
  return {
